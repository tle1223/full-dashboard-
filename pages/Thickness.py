...
if "company" not in st.session_state:
    st.session_state["company"] = "บริษัท PAN"

company = st.selectbox(
    "เลือกบริษัท",
    ["บริษัท PAN", "บริษัท MIKI"],
    index=["บริษัท PAn", "บริษัท MIKI"].index(st.session_state["company"])
)
...
if file:
    ...
    # =============================
        ...
        # ตรวจ Date และสร้าง Month + MonthNum
        if "Date" not in df_miki.columns:
            st.error("❌ ไม่มีคอลัมน์ Date ใน sheet 'thick MIKI'")
            st.stop()
        df_miki = make_month_cols(df_miki, "Date")
